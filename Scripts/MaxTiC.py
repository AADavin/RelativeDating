...
if len(parameters) < 2:
    ...
    print("usage: python MaxTiC.py species_tree_file constraints_file name_output [ls=LOCAL_SEARCH] [t=TEMPERATURE] [r=RANDOMIZATION_TYPE] [d=MIN_DISTANCE] [rd=RANDOM_TREES] [ts=THRESHOLD_CONSTRAINTS]")
    exit()
...
# outputs an topological order of the vertices of the graph
...
print
total_transfers, "total weight of constraints from transfers"
...
# print "search uninformative constraints"

# output informative
edge_keys = edge.keys()
edge_keys.sort(lambda x, y: cmp(edge[y], edge[x]))
sortie = open(name_output + "_WeightedInformativeConstraints.tsv", "w")
for k in edge_keys:
    first = k.split(",")[0]
    second = k.split(",")[1]
...
print
uninformative, "uninformative (", to_desc, "to a descendant,", to_leaf, "to a leaf", to_anc, "to an ancestor", to_itself, "to itself",
print
str(int(uninformative * 100 / (total_transfers + uninformative))) + "%)"
if from_leaf > 0:
    print
    "WARNING: there are constraints from leaves, which cannot be met since this program only orders internal nodes, they are ignored in the scores"
print
trivial_conflict, "trivially conflicting constraints (" + str(
    int(trivial_conflict * 100 / (total_transfers))) + "%) (descendant to ancestor or trivial cycle)"
...
print
"value of the order given by the input tree", value_input, "(" + str((value_input * 100 / total_transfers)) + "%)"
...
for i in range(len(edge_keys)):
    e = edge_keys[i]
    # if (i/10000) * 10000 == i:
    # print i,len(edge_keys)
    words = e.split(",")
    if words[0] == words[1] or path(graph, words[1], words[0]):
        rejected = rejected + edge[e]
    else:
        if not words[1] in graph[words[0]]:
...
print
"greedy solution constructed"

order_greedy = order_from_graph(graph)
value_greedy = value(order_greedy)

print
"value of the greedy heuristic", value_greedy, "(" + str((value_greedy * 100 / total_transfers)) + "%)"
...
print
"value of the mixing heuristic:", value_heuristic, "(" + str((value_heuristic * 100 / total_transfers)) + "%)"
...
print(
tree_from_order(order))
# print order
# print order_input
print
"Similarity of the best order compared with the input order", similarity(order_input, order)
...
if RANDOM > 0:
    ...
    print
    print
    "values from ", RANDOM, " random orders", minimum, maximum
    print
    "pvalue of the found order:", float(pvalue) / RANDOM
    print
    "similarity values from ", RANDOM, "random orders", kendall_tau_min, kendall_tau_max
    print
    "pvalue of the similarity with the input order:", float(kendall_tau_pvalue) / RANDOM

if TIME_FOR_SEARCH > 0:
    print
    "attempting a local search from the best found order, please wait", TIME_FOR_SEARCH, "seconds"
    order = optimisation_locale(order, TIME_FOR_SEARCH)
    print
    "after local search", value(order), "rejected"
    print
    "best found solution", value(order), "(" + str((value(order) * 100 / total_transfers)) + "%)"
    print
    tree_from_order(order)
    print (order)

with open(name_output + "_MaxTiCTree.nwk", "w") as f:
    f.write(tree_from_order(order))
...
sortie = open(name_output + "_MaxTiC_Constraints.tsv", "w")
for e in edge_keys:
    sommets = e.split(",")
    if index.has_key(sommets[0]) and index.has_key(sommets[1]) and index[sommets[0]] < index[sommets[1]] and edge[
        e] < 100000:
        total = total + edge[e]
        if order_input.index(sommets[0]) < order_input.index(sommets[1]):
            sortie.write(str(sommets[0]) + "\t" + str(sommets[1]) + "\t" + str(edge[e])+ "\n")
        else:
            sortie.write(str(sommets[0]) + "\t" + str(sommets[1]) + "\t" + str(edge[e]) + "\n")
            conflict_with_input = conflict_with_input + edge[e]

print(
conflict_with_input / total, " constraints in agreement with the best tree in conflict with input tree")
print(
"Similarity of the order compared with the input order", similarity(order_input, order))
